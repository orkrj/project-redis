...
## 2. Core 모듈 (`core`)
### **목적**
- 애플리케이션의 실행 엔트리포인트를 제공하는 중심 모듈.
- 프로젝트의 메인 실행 환경을 관리.

### **역할**
- 메인 실행 클래스 (`MainApplication`)
- 프로젝트 전역 설정 초기화
...
## 5. Domain 모듈 (`domain`)
### **목적**
- 프로젝트의 핵심 비즈니스 로직 및 엔티티 관리.
- 애플리케이션의 주요 비즈니스 규칙을 정의.

### **역할**
- 도메인 객체 및 엔티티 정의
- 도메인 서비스 및 비즈니스 규칙 구현
- 도메인 객체 간 관계 설정

---

## 6. Infrastructure 모듈 (`infrastructure`)
### **목적**
...
project-movie/
├── common/         # 공통 모듈
├── core/           # 실행 모듈 (메인 클래스 포함)
├── api/            # 컨트롤러 모듈 (외부 인터페이스)
├── application/    # 비즈니스 서비스 처리 및 포트 역할
├── domain/         # 핵심 비즈니스 영역 (엔티티 및 도메인 로직)
